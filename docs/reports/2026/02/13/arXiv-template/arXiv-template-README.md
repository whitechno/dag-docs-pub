# arXiv Paper Template in MLSys Style

A template for LaTeX source to produce a PDF paper in MLSys style.
...
TEXINPUTS=tex-source: pdflatex -output-directory=build tex-source/paper_template.tex && \
TEXINPUTS=tex-source: pdflatex -output-directory=build tex-source/paper_template.tex && \
cp build/paper_template.pdf pub/mlsys-template.pdf && \
rm -rf build
```
or, run `make pdf`.
